...
def analyze_alerts_performance(tokens, thresholds, alert_times):
    ...
    for token in tokens:
        ...
        if df_5m.index.tz is None:  # If the 5m data index is timezone-naive
            df_5m.index = df_5m.index.tz_localize('UTC')  # Localize to UTC

        # Calculate max close price from 5m data AFTER the alert
        df_5m_after_alert = df_5m[df_5m.index >= alert_time_dt]  # Filter rows after the alert time
        if not df_5m_after_alert.empty:
            max_price = df_5m_after_alert['close'].max()
        else:
            max_price = alert_price  # If no data after alert, use alert price as fallback
